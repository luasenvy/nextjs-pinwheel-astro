...
import type { CareerItem, SidebarItem } from "@/lib/data/career";
...
export default function CareerSidebar({ careers, sidebar }: CareerSidebarProps) {
  return (
    <div className="career-single-sidebar mt-8 lg:col-4 lg:mt-0">
      {sidebar.enable && (
        <div className="mb-8 rounded-xl bg-white px-7 py-10 shadow-lg">
          <h5 className="h5">{sidebar.title}</h5>
          <p className="mt-4">{sidebar.content}</p>
          <a className="btn btn-primary mt-6 block w-full" href={sidebar.button.link}>
            {sidebar.button.label}
          </a>
        </div>
      )}
      {careers.slice(0, 2).map(({ metadata }, i) => (
        <div key={`career-${i}`} className="mb-8 rounded-xl bg-white px-7 py-10 shadow-lg">
          <h5 className="h5">{metadata.title}</h5>
          <p className="mt-6">{metadata.excerpt}</p>
