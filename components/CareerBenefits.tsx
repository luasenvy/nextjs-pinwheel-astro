...
export interface CareerBenefitsProps {
  title: string;
  description: string;
  benifit_list: Array<BenefitItem>;
}

export default function CareerBenefits({ title, description, benifit_list }: CareerBenefitsProps) {
  return (
    <section className="section">
      <div className="container">
        ...
        <div className="row mt-14 text-center">
          {benifit_list.map(({ icon, title, color, content }, i) => {
            const key = humanize(icon) as keyof typeof Icon;
            const FeatherIcon = Icon[key];

            return (
              <div className="sm:col-6 lg:col-4 mb-10" key={i}>
                <div className="relative">
                  ...
                  <span className="benifit-icon absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-[28%]">
                    <FeatherIcon color={color} size={48} />
                  </span>
                </div>

                <h3 className="h4 mb-4 mt-8">{title}</h3>
                <p>{content}</p>
              </div>
            );
          })}
